...
__all__ = ['HomoGaussian', 'AffineHomoGaussian', 'NNHomoGaussian',
           'NNHeteroGaussian', 'NNHeteroGaussianIndexed']
...
class NNHeteroGaussianIndexed(Likelihood):

    def __init__(self, in_dim, out_dim, k, hidden_dims=(64, 64),
                 min_sigma=1e-3, init_sigma=None, nonlinearity=F.relu):
        super().__init__()
        ...
        if init_sigma is not None:
            init_sigma *= in_dim ** 0.5
            for network in self.networks:
                network.layers[-1].bias.data[out_dim:] = torch.log(
                    torch.exp(torch.tensor(init_sigma)) - 1)

    def forward(self, z, label):
        """Returns parameters of a diagonal Gaussian distribution."""
        output = torch.zeros((z.shape[0], 2*self.out_dim))
        for k in range(self.k):
            idx = torch.where(label == k)[0]
            output[idx] = self.networks[k](z[idx, :])
